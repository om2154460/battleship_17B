    ...
    ships.resize(5);
        ...
        while (i != 5){
            ...
            if (tempShip.getBadPlacement()){
                qDebug() << "Placement of the " << index2ShipName(i) << " was off the grid. Try again!" << endl;
            ...
            if (coordinates.size() > 0){
                ...
                if (rePlaceShip){
                    qDebug() << "Placement of the " << index2ShipName(i) << " was on top of another ship. Try again!" << endl;
...
vector<Ship> AI_Ship_Placer::setShips3(){
    ...
    //Randomly choose 5 directions (DOWN or RIGHT), one for each coordinate
    directions = getRandomDirections();
    vector<QString> possCoordinates = randomCoordinates();
    int i = 0;
    while (i != 5){
        //Randomly choose a coordinate
        ...
        if (coordinates.size() > 0){
            ...
            if (rePlaceShip){
                qDebug() << "Placement of the " << index2ShipName(i) << " was on top of another ship. Try again!";
                continue;	//The temporary ship was placed on top of another ship, go back to the asking the player for new coordinates.
            }
        }
        ...
        if (debugging){
            QString temp;
            //Display where the Ship was placed
            temp += index2ShipName(i) + " placed on coordinates: ";
            for (int f = 0; f < signed(tempCoordinates.size()); f++){
...
vector<QString> AI_Ship_Placer::getRandomCoordinates(){
    vector<QString> coordinates;
    QString position;
    for (int i = 0; i < 5; i++){
...
QString AI_Ship_Placer::getRandomCoordinate(){
    QString coordinate;
    //Initialize the random seed
    std::srand(unsigned(std::time(NULL)));
    //Generate a random number between 1 and 100
...
QString AI_Ship_Placer::num2Coordinate(int number){
...
QString AI_Ship_Placer::nextPosition(char direction, QString lastHit){
    QString nextPos;
...
int AI_Ship_Placer::strLetter2Num(QString letter){
    QChar cLetter = letter.at(0);
    char d = cLetter.toLatin1();
...
QString AI_Ship_Placer::num2StrLetter(int x){
    //0-9 == A-J
    switch (x){
    case 0: return "A";
    case 1: return "B";
...
int AI_Ship_Placer::strNum2Num(QString num){
    QChar cNum;
    if (num == "10")
        return 10;
    else
...
QString AI_Ship_Placer::intNum2StrNum(int num){
    switch (num){
    case 0: return "0";
    case 1: return "1";
    case 2: return "2";
...
Ship AI_Ship_Placer::fillShip(QString shipName, QString coordinate, char direction){
    Ship ship;
    vector<QString> tempCoordinates;
    QString temp;
...
int AI_Ship_Placer::shipName2Index(QString name){
...
QString AI_Ship_Placer::index2ShipName(int index){
    switch(index){
...
int AI_Ship_Placer::shipName2Size(QString name){
    if (name == "CARRIER")	//Size 5
        return 5;
    if (name == "BATTLESHIP")	//Size 4
        return 4;
...
//Place the ships given the provided starting coordinates and directions
vector<Ship> AI_Ship_Placer::placeShips2(vector<QString> coordinates, vector<char> directions){
    vector<Ship> ships;
    vector<QString> allCoordiantes;
    vector<QString> tempCoordinates;
...
//Place the ships given the provided starting coordinates and directions
vector<Ship> AI_Ship_Placer::placeAllShips(vector<QString> coordinates, vector<char> directions){
    vector<Ship> ships;
    vector<QString> allCoordiantes;
    vector<QString> tempCoordinates;
    std::vector<QString>::iterator it;
...
vector<QString> AI_Ship_Placer::randomCoordinates(){
    ...
    for (int i = 0; i < 10; i++){
        for (int x = 0; x < 10; x++){
            ...
            coordinates[(i * 10) + x] = num2Letter(i) + intNum2StrNum(x + 1);    //Must convert integer to a QString
        }
    }

    //Create temporary move vector and copy vectors
    vector<QString> tempCoordinates(coordinates);
    ...
    //Do the swap!
    QString temp = coordinates[coordinates.size() - 1];
